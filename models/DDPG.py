...
class Actor(object):
	def __init__(self, sess, action_dim, action_bound, learning_rate, replacement, S, R, S_):
		self.sess = sess
...
class DDPG(object):
	def __init__(self, ENV_NAME):
		...
		with tf.name_scope('S'):
			self.S = tf.placeholder(tf.float32, shape=[None, self.state_dim], name='s')
		with tf.name_scope('R'):
			self.R = tf.placeholder(tf.float32, [None, 1], name='r')
		with tf.name_scope('S_'):
			self.S_ = tf.placeholder(tf.float32, shape=[None, self.state_dim], name='s_')
